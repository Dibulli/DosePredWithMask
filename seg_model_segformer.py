...
#        summary(self.model.to(self.device), (6, 512, 512))
    ...
    def train_run(self):
        ...
        for batch_i, batch_data in enumerate(tr_loader):
            ...
            logger.info("true_middle layer: {0: .4f} -- {1: .4f} -- {2: .4f}".format(true_dose[0, 0, 500, 500],
                                                                                    true_dose[0, 0, 512, 512],
                                                                                    true_dose[0, 0, 520, 520]))
            ...
            logger.info("pre_middle layer: {0: .4f} -- {1: .4f} -- {2: .4f}".format(a[0, 0, 500, 500],
                                                                                    a[0, 0, 512, 512],
                                                                                    a[0, 0, 520, 520]))
            ...
            loss_dose_mask = torch.nn.L1Loss()(pred_mat,true_mat)
            loss_without_mask = torch.nn.L1Loss()(pred_out, true_dose)
            loss = loss_dose_mask + loss_without_mask
    ...
    def val_run(self):
        ...
        with torch.no_grad():
            for batch_i, batch_data in enumerate(val_loader):
                ...
                true_vis = np.array(true_dose)
                logger.info("true_dose max in all slices: {0: .3f}".format(np.max(true_vis)))
                logger.info("true_middle layer: {0: .4f} -- {1: .4f} -- {2: .4f}".format(true_dose[0, 0, 500, 500],
                                                                                         true_dose[0, 0, 512, 512],
                                                                                         true_dose[0, 0, 520, 520]))
                ...
                pred_vis = pred_out.cpu().detach().numpy()
                logger.info("pred_dose max in all slices: {0: .3f}".format(np.max(pred_vis)))
                logger.info("pre_middle layer: {0: .4f} -- {1: .4f} -- {2: .4f}".format(pred_vis[0, 0, 500, 500],
                                                                                        pred_vis[0, 0, 512, 512],
                                                                                        pred_vis[0, 0, 520, 520]))
                # masks_pred = pred_out[0]
                # dose_pred = pred_out
